...
SELECT 
  ...
  COALESCE((stats_data->'monthly'->>'distance')::REAL, 0),
  COALESCE((stats_data->'monthly'->>'activitiesCount')::INTEGER, 0),
  COALESCE((stats_data->'monthly'->>'duration')::INTEGER, 0),
  COALESCE((stats_data->'monthly'->'longestActivity'->>'distance')::REAL, 0),
  ...
  COALESCE((stats_data->'latest'->>'distance')::REAL, 0),
...
SELECT 
  ...
  EXTRACT(YEAR FROM CURRENT_DATE)::INTEGER,
  NULL,
  COALESCE((stats_data->'yearly'->>'distance')::REAL, 0),
  COALESCE((stats_data->'yearly'->>'activitiesCount')::INTEGER, 0),
